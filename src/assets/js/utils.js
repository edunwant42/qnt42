...
/**
 * Decrypt a secret key using a password and salt.
 *
 * @param {string} encryptedData The encrypted secret key (Base64 encoded).
 * @param {string} password The password to derive the decryption key.
 * @param {string} salt A unique salt value.
 * @returns {Promise<string>} The decrypted secret key.
 */
export async function decryptSecretKey(encryptedData, password, salt) {
...
/**
 * Encrypt a secret key using a password and salt.
 *
 * @param {string} secretKey The secret key to encrypt.
 * @param {string} password The password to derive the encryption key.
 * @param {string} salt A unique salt value.
 * @returns {Promise<string>} The encrypted secret key (Base64 encoded).
 */
export async function encryptSecretKey(secretKey, password, salt) {
    const key = await deriveKeyFromPassword(password, salt);
    const encoder = new TextEncoder();
